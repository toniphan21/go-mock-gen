...
func (m *repository) Find() UserAlias {
	v0, v1, v2 := "Repository", "Find", "() github.com/you/project.UserAlias"
	v3 := []any{}
...
func (s *repositoryStubber) Find(stub func() UserAlias) *repositoryFind {
	if s.target.td == nil {
	...
	m := s.target.td.Find
	if m == nil {
		m = &repositoryFind{stubLocation: mockgenCallerLocation(2)}
		s.target.td.Find = m
	}
...
func (e *repositoryExpecter) Find(tb testing.TB) *repositoryFindExpecter {
	if e.target.td == nil {
		e.target.td = &repositoryTestDouble{}
	}

	var m = e.target.td.Find
	if m == nil {
		m = &repositoryFind{}
		e.target.td.Find = m
	}
	...
	idx := len(m.expects)
	m.expects = append(m.expects, &repositoryFindExpect{
	...
	return &repositoryFindExpecter{expect: m.expects[idx]}
}

type repositoryFind struct {
	Calls        []repositoryFindCall
	stub         func() UserAlias
	stubLocation string
	expects      []*repositoryFindExpect
	verified     bool
}

func (m *repositoryFind) methodName() string {
	return "Find"
}

func (m *repositoryFind) interfaceName() string {
	return "Repository"
}

func (m *repositoryFind) fatal(index int, msg string) {
	m.verified = true
	m.expects[index].tb.Helper()
	m.expects[index].tb.Fatal(msg)
}

func (m *repositoryFind) panic(msg string) {
	m.verified = true
	panic(msg)
}

func (m *repositoryFind) buildCallHistory(sb *strings.Builder, header string) {
...
func (m *repositoryFind) invokeStub() UserAlias {
	v0 := m.stub()
	return m.capture(repositoryFindReturn{first: v0})
}

func (m *repositoryFind) invokeExpect() UserAlias {
	v0 := []any{}
...
func (m *repositoryFind) capture(returns repositoryFindReturn) UserAlias {
	m.Calls = append(m.Calls, repositoryFindCall{Location: mockgenCallerLocation(4), Return: returns})
	return returns.first
}

func (m *repositoryFind) verify(index int) {
...
type repositoryFindCall struct {
	Location string
	Return   repositoryFindReturn
}

type repositoryFindReturn struct {
	first UserAlias
}

type repositoryFindExpect struct {
	returns  repositoryFindReturn
	location string
	index    int
	tb       testing.TB
}

type repositoryFindExpecter struct {
	expect *repositoryFindExpect
}

func (e *repositoryFindExpecter) Return(first UserAlias) {
	e.expect.returns = repositoryFindReturn{first: first}
}
```
